...
class Func_Class:  
    def __init__(self):
        ...
        checkpoint_path = self.path + '/tensorflow/cp_face.ckpt'
        ...
        if ret: 
            ...
            for face in dets:                
                ...
                try: 
                    ...
                    # print(n_predict[0,np.argmax(n_predict[0])] * 100) 
                    num = (((n_predict[0,np.argmax(n_predict[0])]) * 1000)//1)
                ...
                try:
                    if (n_predict[0,np.argmax(n_predict[0])] * 100) > 80.0:
                        ...
                        draw.text((face.left() - 10, face.top() - 25), text, font = ImageFont.truetype((self.path + "/gui/HiMelody-Regular.ttf"), 25), fill = (0, 255, 0, 0))
...
###노래 관련############################################
    #노래 관련 초기화 
    def song_init(cls): 
        Func_Class.path_dir =  os.path.realpath('.') + '\mp3' #mp3 root 위치 변수
